...
BUILTIN_SHELVES = [
    {"id": "recent", "name": "📥 Recently Added", "query": "ORDER BY b.created_at DESC LIMIT 50", "icon": "📥"},
    {
        ...
        "query": "JOIN reading_progress rp ON rp.book_id = b.id WHERE rp.percentage > 0 AND rp.percentage < 100 ORDER BY rp.updated_at DESC",
        "icon": "📖",
    },
    {
        "id": "unread",
        "name": "🆕 Unread",
        "query": "WHERE NOT EXISTS (SELECT 1 FROM reading_progress rp WHERE rp.book_id = b.id AND rp.percentage > 0) ORDER BY b.created_at DESC LIMIT 100",
        "icon": "🆕",
    },
    ...
    {"id": "top_rated", "name": "⭐ Highest Quality", "query": "WHERE b.quality_score >= 85 ORDER BY b.quality_score DESC", "icon": "⭐"},
    {
        "id": "audiobooks",
        "name": "🎧 Audiobooks",
        "query": "JOIN book_files bf ON bf.book_id = b.id WHERE bf.format IN ('mp3','m4b','m4a','flac')",
        "icon": "🎧",
    },
...
async def get_shelf_books(shelf_id: str, limit: int = 50, offset: int = 0) -> list[dict[str, Any]]:
    ...
    rows = await fetch_all(f"SELECT b.id, b.title, b.isbn, b.cover_path, b.quality_score, b.language FROM books b {query}")
